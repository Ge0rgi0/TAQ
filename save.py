...
class Circles:
    ...
    def shrink_all(self):
        # Fait rapetisser tous les cercles restants (effet cumulé)
        for circle in self.circles:
            # On ne descend pas sous un minimum raisonnable
            circle.radius = max(10, circle.radius - 20)
    ...
    def draw(self, screen):
        if not self.circles[0].visible and self.circles[0].countdown != 1:
            ...
            # Dès qu'un cercle disparaît, on rapetisse tous les autres (effet cumulé)
            self.shrink_all()
...
while running and frame_count < TOTAL_FRAMES:
    ...
    subtext = pygame.font.SysFont(None, 40).render(TEXT_SUBTITLE, True, (0, 0, 0))  # noir
    ...
    # (Supprimé, ne rien afficher en bas)
    # ...existing code...
