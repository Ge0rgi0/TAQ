...
# Physique balles
MAX_SPEED = 2.99
BALL_RADIUS_RATIO = 0.04  # proportion de la largeur
...
TEXT_TITLE = " Who's the most in love with you ?"
TEXT_SUBTITLE = "(It's not gay if it's secret)"
TEXT_YES = "GF"
TEXT_NO = "Bro"
...
CAPTURE_IMAGE_PATH = os.path.join(os.path.dirname(__file__), "capture.png")
capture_img = pygame.image.load(CAPTURE_IMAGE_PATH).convert_alpha()
...
while running and frame_count < TOTAL_FRAMES:
    ...
    # Dernières 5*60 frames : affichage spécial
    if frame_count >= TOTAL_FRAMES - 5*60:
        screen.fill((0, 0, 0))
        # Affiche l'image capture.png centrée
        img_width = SCREEN_WIDTH
        img_height = int(img_width * capture_img.get_height() / capture_img.get_width())
        img_disp = pygame.transform.smoothscale(capture_img, (img_width, img_height))
        img_rect = img_disp.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 - 60))
        screen.blit(img_disp, img_rect)

        # Affiche la réponse gagnante
        ...
        text_rect = text.get_rect(center=(SCREEN_WIDTH // 2, img_rect.bottom + 60))
        ...
        # Rectangle du haut : affiche l'image "capture.png" centrée plus bas
        img_width = SCREEN_WIDTH
        img_height = int(img_width * capture_img.get_height() / capture_img.get_width())
        img_disp = pygame.transform.smoothscale(capture_img, (img_width, img_height))
        img_rect = img_disp.get_rect(centerx=SCREEN_WIDTH // 2, top=120)
        screen.blit(img_disp, img_rect)

        # Deux petits rectangles juste en dessous de l'image
        small_font = pygame.font.SysFont(None, 48)
        rects_top = img_rect.bottom + 20  # Juste en dessous de l'image
