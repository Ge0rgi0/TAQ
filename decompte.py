...
MAX_CIRCLES = 100  # <-- Ajout : définis ici le nombre de cercles
...
class Circles:
    def __init__(self, circles):
        ...
        self.created = len(circles)  # Nombre de cercles déjà créés

    def __iter__(self):
        return iter(self.circles)

    def draw(self, screen):
        # Ne crée un nouveau cercle que si on n'a pas dépassé MAX_CIRCLES
        if self.circles and not self.circles[0].visible and self.circles[0].countdown != 1:
            if self.created < MAX_CIRCLES:
                ...
                # Si on a atteint MAX_CIRCLES, on retire juste le cercle sans en ajouter
                self.circles.pop(0)
...
CIRCLES = Circles([
    ...
    for i, factor in enumerate(CIRCLE_RADIUS_FACTORS[:MAX_CIRCLES])
])
            ...
            if circle.visible or circle.countdown > 0:
                ...
                if prev_visible and not circle.visible:
                    ...
                    if getattr(circle, "difficulty", None) == "easy":
                        ...
                        score_yes += 5
                    remaining_circles = max(remaining_circles - 1, 0)
